 ...
 *     4-7 = RFU
 */
static bool elrs_cmd_check(uint16_t opcode, const uint8_t *params, uint16_t param_len) {
    uint8_t resp[1] = {0};
...
lr1121_status_t lr1121_elrs_init(uint32_t freq_min, uint32_t freq_max) {
    ...
    DEBUGOUT("\n");
    DEBUGOUT("╔═══════════════════════════════════════════════════════════════╗\n");
    DEBUGOUT("║  LR1121 EXTERNAL TCXO INITIALIZATION (Waveshare Core1121-XF)  ║\n");
    DEBUGOUT("║  FIX 2026-01-21: voltage=0 for externally-powered TCXO        ║\n");
    DEBUGOUT("╠═══════════════════════════════════════════════════════════════╣\n");
    DEBUGOUT("║  Hardware: 32MHz TCXO always powered from 3.3V rail           ║\n");
    DEBUGOUT("║  Config: SetTcxoMode(0, ~20ms) → SetStandby(XOSC) → Calibrate ║\n");
    DEBUGOUT("╚═══════════════════════════════════════════════════════════════╝\n");
    DEBUGOUT("\n");
    ...
    DEBUGOUT("[TCXO]   delay=0x%06lX (~%lums stabilization)\n",
             ...
             (unsigned long)(tcxo_delay * 31 / 1000));
    ...
    if (!ok) {
        ...
        DEBUGOUT("[TCXO] Without TCXO mode, LR1121 expects crystal input → init will fail\n");
        return LR1121_ERROR_SPI_INIT;
    }
    DEBUGOUT("[TCXO] SetTcxoMode OK - Chip configured for single-ended TCXO input on XTA\n");
    ...
    if (!ok) {
        ...
        DEBUGOUT("[TCXO] System clock NOT switched to TCXO - RF precision will be poor\n");
        ...
        if (lr1121_elrs_get_status(&chip_mode, &cmd_status, &errors)) {
            DEBUGOUT("[TCXO] GetStatus: chip_mode=%d (expect 2=STDBY_XOSC), cmd_status=%d\n", 
                     chip_mode, cmd_status);
            DEBUGOUT("[TCXO]   If chip_mode=1 (STDBY_RC): TCXO failed to start\n");
            DEBUGOUT("[TCXO]   If chip_mode=0 (SLEEP): Wakeup failed or standby ignored\n");
            
            DEBUGOUT("[TCXO] GetErrors: 0x%04X\n", errors);
            if (errors & 0x0020) DEBUGOUT("[TCXO]   HF_XOSC_START_ERR: TCXO delay/voltage issue!\n");
            if (errors & 0x0080) DEBUGOUT("[TCXO]   PLL_LOCK_ERR: PLL failed to lock\n");
            if (errors & 0x0040) DEBUGOUT("[TCXO]   PLL_CALIB_ERR: PLL calibration failed\n");
        }
    ...
    if (lr1121_elrs_get_status(&verify_mode, &verify_status, &verify_errors)) {
        DEBUGOUT("[TCXO] Verify: chip_mode=%d (expect 2=STDBY_XOSC), errors=0x%04X\n",
                 verify_mode, verify_errors);
        if (verify_mode != 2) {
            DEBUGOUT("[TCXO] WARNING: Not in STDBY_XOSC mode! TCXO may have failed.\n");
            if (verify_errors & 0x0020) {
                DEBUGOUT("[TCXO] HF_XOSC_START_ERR detected - try different voltage/delay!\n");
            }
        }
    }
     ...
     * =========================================================================
     */
    DEBUGOUT("[TCXO] Step 6: Calibrate(0x3F) - Full calibration with stable TCXO\n");
    ...
    DEBUGOUT("[TCXO] Calibrate(0x3F) OK - All blocks calibrated with TCXO reference\n");
     ...
     * =========================================================================
     */
    DEBUGOUT("[TCXO] Step 7: SetStandby(XOSC) - Ensure XOSC mode after calibration\n");
    ok = elrs_cmd_check(ELRS_CMD_SET_STANDBY, &standby_xosc, 1);
    if (!ok) {
     ...
     * Combined = 0x0C
     */
    uint8_t irq_params[8] = {0};  /* ONLY 8 bytes - not 16! */
    
    /* IRQ mask: Only enable TX_DONE and RX_DONE (bits in low byte) */
    irq_params[3] = LR1121_IRQ_TX_DONE | LR1121_IRQ_RX_DONE;  /* 0x0C */
    
    /* DIO1 mask: Route TX_DONE and RX_DONE to DIO1 pin */
    irq_params[7] = LR1121_IRQ_TX_DONE | LR1121_IRQ_RX_DONE;  /* 0x0C */
    
    ok = elrs_cmd_check(ELRS_CMD_SET_DIO_IRQ_PARAMS, irq_params, 8);
    if (!ok) {
    ...
    DEBUGOUT("\n");
    DEBUGOUT("[TCXO] ╔═══════════════════════════════════════════════════════════╗\n");
    DEBUGOUT("[TCXO] ║  EXTERNAL TCXO INITIALIZATION COMPLETE!                   ║\n");
    DEBUGOUT("[TCXO] ╠═══════════════════════════════════════════════════════════╣\n");
    DEBUGOUT("[TCXO] ║  ✓ SetTcxoMode(0, ~20ms) - External TCXO mode enabled     ║\n");
    DEBUGOUT("[TCXO] ║  ✓ SetStandby(XOSC) - Clock switched to TCXO              ║\n");
    DEBUGOUT("[TCXO] ║  ✓ SetRegMode(DCDC) - Power efficiency enabled            ║\n");
    DEBUGOUT("[TCXO] ║  ✓ Calibrate(0x3F) - Full calibration with stable TCXO    ║\n");
    DEBUGOUT("[TCXO] ║  ✓ Chip is now in STANDBY_XOSC mode                       ║\n");
    DEBUGOUT("[TCXO] ╚═══════════════════════════════════════════════════════════╝\n");
    DEBUGOUT("\n");
...
uint32_t lr1121_elrs_get_random(void) {
    ...
    uint32_t rand_val = ((uint32_t)resp[1] << 24) |
                        ((uint32_t)resp[2] << 16) |
                        ((uint32_t)resp[3] << 8) |
                        resp[4];
...
void lr1121_elrs_print_status(void) {
    ...
    static const char* mode_names[] = {
        "SLEEP", "STDBY_RC", "STDBY_XOSC", "FS", "RX", "TX", "?", "?"
    };
    
    /* Command status names */
    static const char* cmd_names[] = {
        "FAIL", "PERR", "SPI_ERR", "OK", "DATA_AVAIL", "?", "?", "?"
    };
    ...
    if (errors != 0 && errors != 0xFFFF) {
        if (errors & 0x0001) DEBUGOUT("    [ERR] LF_RC_CALIB\n");
        if (errors & 0x0002) DEBUGOUT("    [ERR] HF_RC_CALIB\n");
        if (errors & 0x0004) DEBUGOUT("    [ERR] ADC_CALIB\n");
        if (errors & 0x0008) DEBUGOUT("    [ERR] PLL_CALIB\n");
        if (errors & 0x0010) DEBUGOUT("    [ERR] IMG_CALIB\n");
        if (errors & 0x0020) DEBUGOUT("    [ERR] HF_XOSC_START\n");
        if (errors & 0x0040) DEBUGOUT("    [ERR] LF_XOSC_START\n");
        if (errors & 0x0080) DEBUGOUT("    [ERR] PLL_LOCK\n");
        if (errors & 0x0100) DEBUGOUT("    [ERR] RX_ADC_OFFSET\n");
    }
 ...
 * 
 * Based on user's analysis of the "200mV headroom rule" and "ghost RC oscillator"
 * problem that causes Temperature/TCXO tests to fail while RNG works.
 ******************************************************************************/
...
lr1121_status_t lr1121_bulletproof_tcxo_init(uint8_t tcxo_voltage) {
    const char* voltage_names[] = {"1.6V", "1.7V", "1.8V", "2.2V", "2.4V", "2.7V", "3.0V", "3.3V"};
    const char* voltage_str = (tcxo_voltage <= 7) ? voltage_names[tcxo_voltage] : "INVALID";
    ...
    if (tcxo_voltage >= 0x05) {  /* 2.7V or higher */
        ...
        DEBUGOUT("    Note: Waveshare demo uses 3.0V (0x06) which works on their board\n\n");
    }
     ...
     * Citation: Semtech lr11xx_system.c - LR11XX_SYSTEM_ENABLE_SPI_CRC_OC = 0x0108
     * 
    ...
    DEBUGOUT("  Citation: Semtech lr11xx_system.c LR11XX_SYSTEM_ENABLE_SPI_CRC_OC\n");
    ...
    DEBUGOUT("  SPI Frame: [0x01 0x12] [enable] [standby] [rx] [tx] [tx_hp] [tx_hf] [gnss] [wifi]\n");
    ...
    if (!lr1121_send_command(ELRS_CMD_SET_DIO_AS_RF_SWITCH, rf_switch_params, 8)) {
    ...
    DEBUGOUT("    lr11xx_system_set_tcxo_mode(context, LR11XX_SYSTEM_TCXO_CTRL_3_0V, 300);\n");
    ...
    for (volatile uint32_t i = 0; i < 1000000; i++) { }  /* ~100ms delay */
     ...
     * 
     * CRITICAL FIX (2026-01-16): Now uses Internal RC (0x00) instead of XTAL (0x02)
     * 
     * The Waveshare Core1121-HF module may NOT have a 32.768kHz crystal populated.
     * Configuring for LF_XTAL when no crystal exists causes PLL_LOCK_ERR because
     * the LR1121 state machine hangs waiting for a clock that never arrives.
     * 
    ...
    DEBUGOUT("  Citation: Semtech lr11xx_system.c line 368 - single byte encoding\n");
     ...
     * 
     * If board lacks 32kHz XTAL, define LR1121_LFCLK_USE_RC=1 to use RC oscillator.
     */
#if LR1121_LFCLK_USE_RC
    /* Use internal 32kHz RC oscillator (less accurate, but always available) */
    uint8_t lfclk_param = LR11XX_SYSTEM_LFCLK_RC | (0x01 << 2);  /* RC=0x00, wait_32k=bit2 -> 0x04 */
    DEBUGOUT("  Using LFCLK_RC (internal 32kHz RC oscillator)\n");
#else
    /* Use 32kHz crystal (Waveshare default - more accurate) */
    uint8_t lfclk_param = LR11XX_SYSTEM_LFCLK_XTAL | (0x01 << 2);  /* XTAL=0x02, wait_32k=bit2 -> 0x06 */
    ...
    if (lr1121_verify_xosc_mode(&chip_mode)) {
        ...
        if (chip_mode == 0) {
            DEBUGOUT("    Mode 0 = SLEEP: Chip fell back to SLEEP (XOSC failed to start)\n");
            DEBUGOUT("    Check: TCXO hardware, SetTcxoMode voltage setting\n");
        } else if (chip_mode == 1) {
            DEBUGOUT("    Mode 1 = STDBY_RC: TCXO switch FAILED (still on RC oscillator)\n");
