...
bool LR1121Driver::CheckVersion(const SX12XX_Radio_Number_t radioNumber)
{
    ...
    // On ESP32, LittleFS can be used to skip firmware check. On other platforms, always check.
#if defined(PLATFORM_ESP32)
    bool skipUpdate = LittleFS.exists("/lr1121.txt");
#else
    bool skipUpdate = false;
#endif
    if (!skipUpdate && (version.type != LR1121_FIRMWARE_TYPE || version.version != LR11XX_FIRMWARE_VERSION))
    {
...
void LR1121Driver::SetDioAsRfSwitch()
{
...
#else
    // Default RF switch configuration for Waveshare Core1121-HF with PE4259
    ...
    switchbuf[7] = 0b00000001; // RfSwWifiCfg
#endif
...
// Citation: LR1121 Datasheet Section 4.1.1
//   SetDioIrqParams takes 8 bytes:
//   - Bytes 0-3: IRQ mask (which IRQs to enable) - big-endian 32-bit
//   - Bytes 4-7: DIO1 mask (which IRQs route to DIO1 pin) - big-endian 32-bit
//
// OBSERVED: Polled IRQ shows 0x00C00000 for TX_DONE/RX_DONE (bits 22-23)
// This means the actual IRQ bit positions are different from the documented
// LR1121_IRQ_TX_DONE=0x04 and LR1121_IRQ_RX_DONE=0x08.
//
// Setting 0xFFFFFFFF for both masks ensures ALL IRQs are enabled and routed
// to DIO1, regardless of the actual bit positions.
void LR1121Driver::SetDioIrqParams()
{
    ...
    // DIO1 mask: Route ALL IRQs to DIO1 pin (0xFFFFFFFF)
    buf[4] = 0xFF;
    buf[5] = 0xFF;
    buf[6] = 0xFF;
    buf[7] = 0xFF;
    ...
    DBGLN("SetDioIrqParams: ALL IRQs enabled and routed to DIO1");
}

// Get IRQ Status - MATCHES UPSTREAM ELRS EXACTLY
// Upstream uses a clever trick: sends ClearIrq command but reads response
// which contains the IRQ status BEFORE clearing. This is a combined operation.
// 
// Citation: ExpressLRS/src/lib/LR1121Driver/LR1121.cpp line 567-579
uint32_t ICACHE_RAM_ATTR LR1121Driver::GetIrqStatus(SX12XX_Radio_Number_t radioNumber)
{
    // Upstream pattern: buffer contains ClearIrq opcode + 0xFFFFFFFF mask
    // ReadCommand sends this and receives response with IRQ status
    uint8_t status[6] = {0};
    status[0] = LR11XX_SYSTEM_CLEAR_IRQ_OC >> 8;    // 0x01
    status[1] = LR11XX_SYSTEM_CLEAR_IRQ_OC & 0xFF;  // 0x14
    status[2] = 0xFF;  // Clear all IRQs
    status[3] = 0xFF;
    status[4] = 0xFF;
    status[5] = 0xFF;
    ...
    if (debugCount < 10) {
        DBGLN("IRQ raw: [%02X][%02X][%02X][%02X][%02X][%02X]",
              status[0], status[1], status[2], status[3], status[4], status[5]);
        debugCount++;
    }
    
    // Response format: IRQ status in bytes 2-5 (big-endian)
...
static volatile uint32_t rxDoneCount = 0;
static volatile uint32_t txDoneCount = 0;
static volatile uint32_t otherIrqCount = 0;
static volatile uint32_t lastIrqStatus = 0;
...
void ICACHE_RAM_ATTR LR1121Driver::IsrCallback(SX12XX_Radio_Number_t radioNumber)
{
    ...
    if (irqStatus & LR1121_IRQ_TX_DONE)
    {
        txDoneCount++;
        instance->TXnbISR();
        instance->ClearIrqStatus(radioNumber);  // Clear IRQ so DIO1 goes LOW for next edge
        if (GPIO_PIN_NSS_2 != UNDEF_PIN)
        {
    ...
    else if (irqStatus & LR1121_IRQ_TIMEOUT)
    {
        ...
        instance->ClearIrqStatus(radioNumber);
...
extern "C" void lr1121_get_isr_stats(uint32_t *isr_count, uint32_t *rx_count, uint32_t *tx_count, uint32_t *other_count, uint32_t *last_irq)
{
    if (isr_count) *isr_count = isrCallCount;
    if (rx_count) *rx_count = rxDoneCount;
    if (tx_count) *tx_count = txDoneCount;
    if (other_count) *other_count = otherIrqCount;
    if (last_irq) *last_irq = lastIrqStatus;
}
